...
{
  /* Initialize the real and imaginary parts of the scalar field with an analytic profile */
  ...
  // Radially Gaussian with monopolar/dipolar angular dependence, centered at the origin
  ...
  if( monopole_statement ) {
    *phi_re = ( 1. / sqrt(4*Pi) ) * ampSF
                * exp( - deltR_w * deltR_w );
    *phi_im = 0.;
  } else if( dipole_statement ) {
    *phi_re = - ampSF * sqrt( 1.5 / 4. / Pi ) * stheta * cphi
                * exp( - deltR_w * deltR_w );
    *phi_im = - ampSF * sqrt( 1.5 / 4. / Pi ) * stheta * sphi
                * exp( - deltR_w * deltR_w );
  } else {
...
void conf_flat_analytic_SF_source_term (CCTK_REAL x, CCTK_REAL y, CCTK_REAL z,
        ...
        CCTK_REAL *sourceSF1, CCTK_REAL *sourceSF2,
        CCTK_REAL Dphi_re_dx[3], CCTK_REAL Dphi_im_dx[3])
{
