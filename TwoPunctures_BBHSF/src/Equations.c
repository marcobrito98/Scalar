...
BBHSF_BY_KKofxyz (CCTK_REAL x, CCTK_REAL y, CCTK_REAL z)
{
  ...
  for (i = 0; i < 3; i++)
  {
    np_Pp += n_plus[i]  * par_P_plus[i]; nm_Pm += n_minus[i] * par_P_minus[i];
  }
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    {				/* Bowen-York-Curvature :*/
      Aij =
        + 1.5 * ( par_P_plus[i] * n_plus[j]
                + par_P_plus[j] * n_plus[i]
                     + np_Pp * n_plus[i]  * n_plus[j])  / r2_plus
        + 1.5 * ( par_P_minus[i] * n_minus[j]
		 ...
		 CCTK_REAL y, CCTK_REAL z, derivs U, CCTK_REAL *values)
{
  ...
  // Initialize the real and imaginary parts of the scalar field and its derivatives
  ...
  // With backreaction on, we use these functions to fill in the scalar field itself
  // and its contributions to H
  if (switch_on_backreaction){
    ...
    conf_flat_analytic_SF_source_term (x, y, z, phi_re, phi_im,
            &sourceSF1, &sourceSF2, &Dphi_re_dx, &Dphi_im_dx);
