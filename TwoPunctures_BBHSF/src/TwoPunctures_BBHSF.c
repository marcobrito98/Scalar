...
/* Swap two variables */
static inline void swap(CCTK_REAL *restrict const a,
...
static void set_initial_guess(CCTK_POINTER_TO_CONST cctkGH, derivs v) {
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++) {
          i3D = BBHSF_Index(ivar, i, j, k, 1, n1, n2, n3);
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++) {
          indx = BBHSF_Index(ivar, i, j, k, 1, n1, n2, n3);
  ...
  if (do_initial_debug_output && CCTK_MyProc(cctkGH) == 0) {
    ...
    for (ivar = 0; ivar < nvar; ivar++)
      for (i = 0; i < n1; i++)
        for (j = 0; j < n2; j++) {
          ...
          B = -cos(be);
          phi = 0.0;
          indx = BBHSF_Index(ivar, i, j, 0, 1, n1, n2, n3);
          ...
          BBHSF_AB_To_XR(nvar, A, B, &X, &R, U);
          /* Calculation of (x,r)*/
          BBHSF_C_To_c(nvar, X, R, &(s_x[indx]), &r, U);
          /* Calculation of (y,z)*/
          BBHSF_rx3_To_xyz(nvar, s_x[i3D], r, phi, &(s_y[indx]), &(s_z[indx]),
                           U);
          fprintf(debug_file,
    ...
    for (i = n2 - 10; i < n2; i++) {
      CCTK_REAL d;
      indx = BBHSF_Index(0, 0, i, 0, 1, n1, n2, n3);
      d = BBHSF_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v, s_x[indx],
                                           0.0, 0.0);
    ...
    for (i = n2 - 10; i < n2 - 1; i++) {
      CCTK_REAL d;
      int ip = BBHSF_Index(0, 0, i + 1, 0, 1, n1, n2, n3);
      indx = BBHSF_Index(0, 0, i, 0, 1, n1, n2, n3);
      for (j = -10; j < 10; j++) {
        d = BBHSF_PunctIntPolAtArbitPosition(
...
void TwoPunctures_BBHSF(CCTK_ARGUMENTS) {
  ...
  *mm = par_m_minus;
  ...
  if (!F) {
    ...
    BBHSF_allocate_derivs(&u, ntotal);
    BBHSF_allocate_derivs(&v, ntotal);
    BBHSF_allocate_derivs(&cf_v, ntotal);
    ...
    if (!(give_bare_mass)) {
      ...
      do {
        CCTK_VInfo(CCTK_THORNSTRING, "Bare masses: mp=%.15g, mm=%.15g",
                   (double)*mp, (double)*mm);
        BBHSF_Newton(cctkGH, nvar, n1, n2, n3, v, Newton_tol, 1);
    ...
    BBHSF_Newton(cctkGH, nvar, n1, n2, n3, v, Newton_tol, Newton_maxit);

    BBHSF_F_of_v(cctkGH, nvar, n1, n2, n3, v, F, u);

    BBHSF_SpecCoef(n1, n2, n3, 0, v.d0, cf_v.d0);
    ...
    *J2 = center_offset[2] * par_P_minus[0] -
          ...
          center_offset[2] * par_P_plus[0] - center_offset[0] * par_P_plus[2] -
          par_b * par_P_plus[2] + par_S_minus[1] + par_S_plus[1];
    *J3 = -(center_offset[1] * par_P_minus[0]) +
          center_offset[0] * par_P_minus[1] - par_b * par_P_minus[1] -
          center_offset[1] * par_P_plus[0] + center_offset[0] * par_P_plus[1] +
          par_b * par_P_plus[1] + par_S_minus[2] + par_S_plus[2];
  }
  ...
  for (int k = imin[2]; k < imax[2]; ++k) {
    for (int j = imin[1]; j < imax[1]; ++j) {
      for (int i = imin[0]; i < imax[0]; ++i) {
        ...
        /*=== initialize SF grid functions as zero ==================*/
        phi1[ind] = 0.0;
        phi2[ind] = 0.0;
        Kphi1[ind] = 0.0;
        Kphi2[ind] = 0.0;
        ...
        CCTK_REAL Aij[3][3];
        BBHSF_BY_Aijofxyz(xx, yy, zz, Aij);
