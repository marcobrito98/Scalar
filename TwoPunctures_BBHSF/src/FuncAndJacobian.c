...
/* --------------------------------------------------------------------------*/
int
BBHSF_Index (int ivar, int i, int j, int k, int nvar, int n1, int n2, int n3)
{
  int i1 = i, j1 = j, k1 = k;
...
/* --------------------------------------------------------------------------*/
void
BBHSF_allocate_derivs (derivs * v, int n)
{
  int m = n - 1;
  (*v).d0 = dvector (0, m);
  (*v).d1 = dvector (0, m);
...
/* --------------------------------------------------------------------------*/
void
BBHSF_free_derivs (derivs * v, int n)
{
  int m = n - 1;
...
/* --------------------------------------------------------------------------*/
void
BBHSF_Derivatives_AB3 (int nvar, int n1, int n2, int n3, derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
	...
	for (i = 0; i < n1; i++)
	{
	  indx[i] = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[i] = v.d0[indx[i]];
	}
	chebft_Zeros (p, n1, 0);
	chder (p, dp, n1);
	...
	for (j = 0; j < n2; j++)
	{
	  indx[j] = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[j] = v.d0[indx[j]];
	  q[j] = v.d1[indx[j]];
	}
	chebft_Zeros (p, n2, 0);
	...
	for (k = 0; k < n3; k++)
	{
	  indx[k] = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  p[k] = v.d0[indx[k]];
	  q[k] = v.d1[indx[k]];
	  r[k] = v.d2[indx[k]];
	}
...
/* --------------------------------------------------------------------------*/
void
BBHSF_F_of_v (CCTK_POINTER_TO_CONST cctkGH,
  ...
  if (use_sources)
  {
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = BBHSF_Index(0,i,j,k,1,n1,n2,n3);
          ...
          for (ivar = 0; ivar < nvar; ivar++)
          {
            indx = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
          ...
          BBHSF_AB_To_XR (nvar, A, B, &X, &R, U);
          /* Calculation of (x,r) and*/
          /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
          BBHSF_C_To_c (nvar, X, R, &(s_x[i3D]), &r, U);
          /* Calculation of (y,z) and*/
          /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
          BBHSF_rx3_To_xyz (nvar, s_x[i3D], r, phi, &(s_y[i3D]), &(s_z[i3D]), U);
        }
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
          sources[BBHSF_Index(0,i,j,k,1,n1,n2,n3)]=0.0;

  BBHSF_Derivatives_AB3 (nvar, n1, n2, n3, v);
  ...
  for (i = 0; i < n1; i++)
  {
    for (j = 0; j < n2; j++)
    {
      for (k = 0; k < n3; k++)
      {
        ...
        /* Calculation of (x,r) and*/
        /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
        BBHSF_C_To_c (nvar, X, R, &x, &r, U);
        /* Calculation of (y,z) and*/
        /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
        BBHSF_rx3_To_xyz (nvar, x, r, phi, &y, &z, U);
        BBHSF_NonLinEquations (sources[BBHSF_Index(0,i,j,k,1,n1,n2,n3)],
...
/* --------------------------------------------------------------------------*/
void
BBHSF_J_times_dv (int nvar, int n1, int n2, int n3, derivs dv,
...
#pragma omp parallel for private (values,dU,U,i,j,k,al,A,be,B,phi,X,R,x,r,y,z,Am1,ivar,indx) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	BBHSF_AB_To_XR (nvar, A, B, &X, &R, dU);
	/* Calculation of (x,r) and*/
	/* (dU, dU_x, dU_r, dU_3, dU_xx, dU_xr, dU_x3, dU_rr, dU_r3, dU_33)*/
	BBHSF_C_To_c (nvar, X, R, &x, &r, dU);
	/* Calculation of (y,z) and*/
	/* (dU, dU_x, dU_y, dU_z, dU_xx, dU_xy, dU_xz, dU_yy, dU_yz, dU_zz)*/
	BBHSF_rx3_To_xyz (nvar, x, r, phi, &y, &z, dU);
	BBHSF_LinEquations (A, B, X, R, x, r, phi, y, z, dU, U, values);
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  indx = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
BBHSF_JFD_times_dv (int i, int j, int k, int nvar, int n1, int n2,
...
{				/* Calculates rows of the vector 'J(FD)*dv'.*/
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    int iccc = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3),
      ipcc = BBHSF_Index (ivar, i + 1, j, k, nvar, n1, n2, n3),
      imcc = BBHSF_Index (ivar, i - 1, j, k, nvar, n1, n2, n3),
      icpc = BBHSF_Index (ivar, i, j + 1, k, nvar, n1, n2, n3),
      icmc = BBHSF_Index (ivar, i, j - 1, k, nvar, n1, n2, n3),
      iccp = BBHSF_Index (ivar, i, j, k + 1, nvar, n1, n2, n3),
      iccm = BBHSF_Index (ivar, i, j, k - 1, nvar, n1, n2, n3),
      icpp = BBHSF_Index (ivar, i, j + 1, k + 1, nvar, n1, n2, n3),
      icmp = BBHSF_Index (ivar, i, j - 1, k + 1, nvar, n1, n2, n3),
      icpm = BBHSF_Index (ivar, i, j + 1, k - 1, nvar, n1, n2, n3),
      icmm = BBHSF_Index (ivar, i, j - 1, k - 1, nvar, n1, n2, n3),
      ipcp = BBHSF_Index (ivar, i + 1, j, k + 1, nvar, n1, n2, n3),
      imcp = BBHSF_Index (ivar, i - 1, j, k + 1, nvar, n1, n2, n3),
      ipcm = BBHSF_Index (ivar, i + 1, j, k - 1, nvar, n1, n2, n3),
      imcm = BBHSF_Index (ivar, i - 1, j, k - 1, nvar, n1, n2, n3),
      ippc = BBHSF_Index (ivar, i + 1, j + 1, k, nvar, n1, n2, n3),
      impc = BBHSF_Index (ivar, i - 1, j + 1, k, nvar, n1, n2, n3),
      ipmc = BBHSF_Index (ivar, i + 1, j - 1, k, nvar, n1, n2, n3),
      immc = BBHSF_Index (ivar, i - 1, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
BBHSF_SetMatrix_JFD (int nvar, int n1, int n2, int n3, derivs u,
	       int *ncols, int **cols, CCTK_REAL **Matrix)
{
  ...
  values = dvector (0, nvar - 1);
  BBHSF_allocate_derivs (&dv, ntotal);
...
#pragma omp parallel for private (i,j,k,ivar,row) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  row = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  ncols[row] = 0;
	  dv.d0[row] = 0;
	}
      }
  ...
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  column = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
	  dv.d0[column] = 1;
...
/*					i_0 = 0;
	  ...
	  for (i1 = i_0; i1 <= i_1; i1++)
	  {
	    for (j1 = j_0; j1 <= j_1; j1++)
	    {
	      ...
	      {
		BBHSF_JFD_times_dv (i1, j1, k1, nvar, n1, n2, n3,
			      dv, u, values);
		for (ivar1 = 0; ivar1 < nvar; ivar1++)
		{
		  if (values[ivar1] != 0)
		  {
		    row = BBHSF_Index (ivar1, i1, j1, k1, nvar, n1, n2, n3);
		    mcol = ncols[row];
...
CCTK_REAL
BBHSF_PunctEvalAtArbitPosition (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi,
...
/* --------------------------------------------------------------------------*/
void
BBHSF_calculate_derivs (int i, int j, int k, int ivar, int nvar, int n1, int n2,
		  int n3, derivs v, derivs vv)
{
  ...
  vv.d0[0] = v.d0[BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d1[0] = v.d1[BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d2[0] = v.d2[BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  ...
  vv.d23[0] = v.d23[BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
...
CCTK_REAL
BBHSF_PunctTaylorExpandAtArbitPosition (int ivar, int nvar, int n1,
                                  ...
                                  CCTK_REAL z)
{
  ...
  BBHSF_calculate_derivs (i, j, k, ivar, nvar, n1, n2, n3, v, vv);
  result = BBHSF_interpol (a, b, c, vv);
  BBHSF_free_derivs (&vv, 1);
...
/* Calculates the value of v at an arbitrary position (x,y,z)*/
CCTK_REAL
BBHSF_PunctIntPolAtArbitPosition (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
...
CCTK_REAL 
BBHSF_PunctEvalAtArbitPositionFast (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi, int nvar, int n1, int n2, int n3)
{
  int i, j, k, N;
  CCTK_REAL *p, *values1, **values2, result;
...
/* --------------------------------------------------------------------------*/
CCTK_REAL
BBHSF_PunctIntPolAtArbitPositionFast (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
...
void BBHSF_SpecCoef(int n1, int n2, int n3, int ivar, CCTK_REAL *v, CCTK_REAL *cf)
{
  DECLARE_CCTK_PARAMETERS;
