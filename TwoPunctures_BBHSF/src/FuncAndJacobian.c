...
#pragma omp parallel for private(values, dU, U, i, j, k, al, A, be, B, phi, X, \
                                 R, x, r, y, z, Am1, ivar, indx)               \
    schedule(dynamic)
  for (i = 0; i < n1; i++) {
    values = dvector(0, nvar - 1);
          ...
          for (i1 = i_0; i1 <= i_1; i1++) {
            for (j1 = j_0; j1 <= j_1; j1++) {
              for (k1 = k_0; k1 <= k_1; k1++) {
                BBHSF_JFD_times_dv(i1, j1, k1, nvar, n1, n2, n3, dv, u, values);
...
BBHSF_PunctEvalAtArbitPosition(CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B,
                               CCTK_REAL phi, int nvar, int n1, int n2,
                               int n3) {
  int i, j, k, N;
 ...
 * routine */
CCTK_REAL
BBHSF_PunctEvalAtArbitPositionFast(CCTK_REAL *v, int ivar, CCTK_REAL A,
...
/* --------------------------------------------------------------------------*/
CCTK_REAL
BBHSF_PunctIntPolAtArbitPositionFast(int ivar, int nvar, int n1, int n2, int n3,
                                     derivs v, CCTK_REAL x, CCTK_REAL y,
                                     CCTK_REAL z) {
  DECLARE_CCTK_PARAMETERS;
  CCTK_REAL xs, ys, zs, rs2, phi, X, R, A, B, aux1, aux2, result, Ui;
