              ...
              CCTK_REAL const * restrict const * restrict const JFD)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    for (i = 0; i < n1; i++)
    {
      Ip = BBHSF_Index (ivar, i + 1, j, k, nvar, n1, n2, n3);
      Ic = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
      Im = BBHSF_Index (ivar, i - 1, j, k, nvar, n1, n2, n3);
              ...
              CCTK_REAL const * restrict const * restrict const JFD)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    for (j = 0; j < n2; j++)
    {
      Ip = BBHSF_Index (ivar, i, j + 1, k, nvar, n1, n2, n3);
      Ic = BBHSF_Index (ivar, i, j, k, nvar, n1, n2, n3);
      Im = BBHSF_Index (ivar, i, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
BBHSF_TestRelax (CCTK_POINTER_TO_CONST cctkGH,
	   ...
	   CCTK_REAL *dv)
{
  ...
  BBHSF_SetMatrix_JFD (nvar, n1, n2, n3, u, ncols, cols, JFD);
          ...
          CCTK_REAL * restrict const normres)
{
  ...
  BBHSF_allocate_derivs (&ph, ntotal);
/*      ph  = dvector(0, ntotal-1);*/
  rt = dvector (0, ntotal - 1);
  s = dvector (0, ntotal - 1);
  BBHSF_allocate_derivs (&sh, ntotal);
/*      sh  = dvector(0, ntotal-1);*/
  ...
  BBHSF_J_times_dv (nvar, n1, n2, n3, dv, r, u);
#pragma omp parallel for
  for (int j = 0; j < ntotal; j++)
    rt[j] = r[j] = F[j] - r[j];
...
/* -------------------------------------------------------------------*/
void
BBHSF_Newton (CCTK_POINTER_TO_CONST const cctkGH,
        int const nvar, int const n1, int const n2, int const n3,
	derivs v,
