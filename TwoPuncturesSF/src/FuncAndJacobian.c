/* TwoPuncturesSF:  File  "FuncAndJacobian.c"*/
...
/* --------------------------------------------------------------------------*/
int
IndexSF (int ivar, int i, int j, int k, int nvar, int n1, int n2, int n3)
{
  int i1 = i, j1 = j, k1 = k;
...
/* --------------------------------------------------------------------------*/
void
allocate_derivsSF (derivs * v, int n)
{
  int m = n - 1;
  (*v).d0 = dvectorSF (0, m);
  (*v).d1 = dvectorSF (0, m);
...
/* --------------------------------------------------------------------------*/
void
free_derivsSF (derivs * v, int n)
{
  int m = n - 1;
  free_dvectorSF ((*v).d0, 0, m);
  free_dvectorSF ((*v).d1, 0, m);
  free_dvectorSF ((*v).d2, 0, m);
  free_dvectorSF ((*v).d3, 0, m);
  free_dvectorSF ((*v).d11, 0, m);
  free_dvectorSF ((*v).d12, 0, m);
  free_dvectorSF ((*v).d13, 0, m);
  free_dvectorSF ((*v).d22, 0, m);
  free_dvectorSF ((*v).d23, 0, m);
  free_dvectorSF ((*v).d33, 0, m);
}

/* --------------------------------------------------------------------------*/
void
Derivatives_AB3SF (int nvar, int n1, int n2, int n3, derivs v)
{
  int i, j, k, ivar, N, *indx;
  CCTK_REAL *p, *dp, *d2p, *q, *dq, *r, *dr;

  N = maximum3SF (n1, n2, n3);
  p = dvectorSF (0, N);
  dp = dvectorSF (0, N);
  d2p = dvectorSF (0, N);
  q = dvectorSF (0, N);
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
	...
	for (i = 0; i < n1; i++)
	{
	  indx[i] = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
	  p[i] = v.d0[indx[i]];
	}
	chebft_ZerosSF (p, n1, 0);
	chderSF (p, dp, n1);
	chderSF (dp, d2p, n1);
	chebft_ZerosSF (dp, n1, 1);
	chebft_ZerosSF (d2p, n1, 1);
	for (i = 0; i < n1; i++)
	{
	  v.d1[indx[i]] = dp[i];
	  v.d11[indx[i]] = d2p[i];
	...
	for (j = 0; j < n2; j++)
	{
	  indx[j] = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
	  p[j] = v.d0[indx[j]];
	  q[j] = v.d1[indx[j]];
	}
	chebft_ZerosSF (p, n2, 0);
	chebft_ZerosSF (q, n2, 0);
	chderSF (p, dp, n2);
	chderSF (dp, d2p, n2);
	chderSF (q, dq, n2);
	chebft_ZerosSF (dp, n2, 1);
	chebft_ZerosSF (d2p, n2, 1);
	chebft_ZerosSF (dq, n2, 1);
	for (j = 0; j < n2; j++)
	{
	  v.d2[indx[j]] = dp[j];
	  v.d22[indx[j]] = d2p[j];
	...
	for (k = 0; k < n3; k++)
	{
	  indx[k] = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
	  p[k] = v.d0[indx[k]];
	  q[k] = v.d1[indx[k]];
	  r[k] = v.d2[indx[k]];
	}
  ...
  free_dvectorSF (p, 0, N);
  free_dvectorSF (dp, 0, N);
  free_dvectorSF (d2p, 0, N);
  free_dvectorSF (q, 0, N);
  free_dvectorSF (dq, 0, N);
  free_dvectorSF (r, 0, N);
  free_dvectorSF (dr, 0, N);
  free_ivectorSF (indx, 0, N);
}

/* --------------------------------------------------------------------------*/
void
F_of_vSF (CCTK_POINTER_TO_CONST cctkGH,
  ...
  if (use_sources)
  {
    ...
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = IndexSF(0,i,j,k,1,n1,n2,n3);
          ...
          for (ivar = 0; ivar < nvar; ivar++)
          {
            indx = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
  ...
  for (i = 0; i < n1; i++)
  {
    for (j = 0; j < n2; j++)
    {
      for (k = 0; k < n3; k++)
      {
        ...
        /* Calculation of (x,r) and*/
        /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
        C_To_cSF (nvar, X, R, &x, &r, U);
        /* Calculation of (y,z) and*/
        /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
        rx3_To_xyzSF (nvar, x, r, phi, &y, &z, U);
        NonLinEquationsSF (sources[IndexSF(0,i,j,k,1,n1,n2,n3)],
...
/* --------------------------------------------------------------------------*/
void
J_times_dvSF (int nvar, int n1, int n2, int n3, derivs dv,
...
#pragma omp parallel for private (values,dU,U,i,j,k,al,A,be,B,phi,X,R,x,r,y,z,Am1,ivar,indx) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	AB_To_XRSF (nvar, A, B, &X, &R, dU);
	/* Calculation of (x,r) and*/
	/* (dU, dU_x, dU_r, dU_3, dU_xx, dU_xr, dU_x3, dU_rr, dU_r3, dU_33)*/
	C_To_cSF (nvar, X, R, &x, &r, dU);
	/* Calculation of (y,z) and*/
	/* (dU, dU_x, dU_y, dU_z, dU_xx, dU_xy, dU_xz, dU_yy, dU_yz, dU_zz)*/
	rx3_To_xyzSF (nvar, x, r, phi, &y, &z, dU);
	LinEquationsSF (A, B, X, R, x, r, phi, y, z, dU, U, values);
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  indx = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
JFD_times_dvSF (int i, int j, int k, int nvar, int n1, int n2,
	      int n3, derivs dv, derivs u, CCTK_REAL *values)
{				/* Calculates rows of the vector 'J(FD)*dv'.*/
  /* First row to be calculated: row = IndexSF(0,      i, j, k; nvar, n1, n2, n3)*/
  /* Last  row to be calculated: row = IndexSF(nvar-1, i, j, k; nvar, n1, n2, n3)*/
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    int iccc = IndexSF (ivar, i, j, k, nvar, n1, n2, n3),
      ipcc = IndexSF (ivar, i + 1, j, k, nvar, n1, n2, n3),
      imcc = IndexSF (ivar, i - 1, j, k, nvar, n1, n2, n3),
      icpc = IndexSF (ivar, i, j + 1, k, nvar, n1, n2, n3),
      icmc = IndexSF (ivar, i, j - 1, k, nvar, n1, n2, n3),
      iccp = IndexSF (ivar, i, j, k + 1, nvar, n1, n2, n3),
      iccm = IndexSF (ivar, i, j, k - 1, nvar, n1, n2, n3),
      icpp = IndexSF (ivar, i, j + 1, k + 1, nvar, n1, n2, n3),
      icmp = IndexSF (ivar, i, j - 1, k + 1, nvar, n1, n2, n3),
      icpm = IndexSF (ivar, i, j + 1, k - 1, nvar, n1, n2, n3),
      icmm = IndexSF (ivar, i, j - 1, k - 1, nvar, n1, n2, n3),
      ipcp = IndexSF (ivar, i + 1, j, k + 1, nvar, n1, n2, n3),
      imcp = IndexSF (ivar, i - 1, j, k + 1, nvar, n1, n2, n3),
      ipcm = IndexSF (ivar, i + 1, j, k - 1, nvar, n1, n2, n3),
      imcm = IndexSF (ivar, i - 1, j, k - 1, nvar, n1, n2, n3),
      ippc = IndexSF (ivar, i + 1, j + 1, k, nvar, n1, n2, n3),
      impc = IndexSF (ivar, i - 1, j + 1, k, nvar, n1, n2, n3),
      ipmc = IndexSF (ivar, i + 1, j - 1, k, nvar, n1, n2, n3),
      immc = IndexSF (ivar, i - 1, j - 1, k, nvar, n1, n2, n3);
...
/* --------------------------------------------------------------------------*/
void
SetMatrix_JFDSF (int nvar, int n1, int n2, int n3, derivs u,
...
#pragma omp parallel for private (i,j,k,ivar,row) schedule(dynamic)
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  row = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
	  ncols[row] = 0;
	  dv.d0[row] = 0;
	}
      }
  ...
  for (i = 0; i < n1; i++)
  {
	...
	for (ivar = 0; ivar < nvar; ivar++)
	{
	  column = IndexSF (ivar, i, j, k, nvar, n1, n2, n3);
	  dv.d0[column] = 1;

	  i_0 = maximum2SF (0, i - 1);
	  i_1 = minimum2SF (N1, i + 1);
	  j_0 = maximum2SF (0, j - 1);
	  j_1 = minimum2SF (N2, j + 1);
	  k_0 = k - 1;
	  k_1 = k + 1;
/*					i_0 = 0;
					i_1 = N1;
	  ...
	  for (i1 = i_0; i1 <= i_1; i1++)
	  {
	    for (j1 = j_0; j1 <= j_1; j1++)
	    {
	      ...
	      {
		JFD_times_dvSF (i1, j1, k1, nvar, n1, n2, n3,
			      dv, u, values);
		for (ivar1 = 0; ivar1 < nvar; ivar1++)
		{
		  if (values[ivar1] != 0)
		  {
		    row = IndexSF (ivar1, i1, j1, k1, nvar, n1, n2, n3);
		    mcol = ncols[row];
...
CCTK_REAL
PunctEvalAtArbitPositionSF (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi,
			  int nvar, int n1, int n2, int n3)
{
  ...
  N = maximum3SF (n1, n2, n3);
  p = dvectorSF (0, N);
  values1 = dvectorSF (0, N);
  values2 = dmatrixSF (0, N, 0, N);
  ...
  for (k = 0; k < n3; k++)
  {
	...
	p[i] = v[ivar + nvar * (i + n1 * (j + n2 * k))];
      chebft_ZerosSF (p, n1, 0);
      values2[j][k] = chebevSF (-1, 1, p, n1, A);
    }
  }
  ...
  for (k = 0; k < n3; k++)
  {
    for (j = 0; j < n2; j++)
      p[j] = values2[j][k];
    chebft_ZerosSF (p, n2, 0);
    values1[k] = chebevSF (-1, 1, p, n2, B);
  }

  fourftSF (values1, n3, 0);
  result = fourevSF (values1, n3, phi);

  free_dvectorSF (p, 0, N);
  free_dvectorSF (values1, 0, N);
  free_dmatrixSF (values2, 0, N, 0, N);
...
/* --------------------------------------------------------------------------*/
void
calculate_derivsSF (int i, int j, int k, int ivar, int nvar, int n1, int n2,
		  int n3, derivs v, derivs vv)
{
  ...
  vv.d0[0] = v.d0[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d1[0] = v.d1[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d2[0] = v.d2[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d3[0] = v.d3[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d11[0] = v.d11[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_al
    + v.d1[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * cos_al;
  vv.d12[0] =
    v.d12[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al * sin_be;
  vv.d13[0] = v.d13[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d22[0] = v.d22[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_be
    + v.d2[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * cos_be;
  vv.d23[0] = v.d23[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d33[0] = v.d33[IndexSF (ivar, i, j, k, nvar, n1, n2, n3)];
}

/* --------------------------------------------------------------------------*/
CCTK_REAL
interpolSF (CCTK_REAL a, CCTK_REAL b, CCTK_REAL c, derivs v)
{
  return v.d0[0]
...
CCTK_REAL
PunctTaylorExpandAtArbitPositionSF (int ivar, int nvar, int n1,
...
/* Calculates the value of v at an arbitrary position (x,y,z)*/
CCTK_REAL
PunctIntPolAtArbitPositionSF (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  ...
  result = PunctEvalAtArbitPositionSF (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
CCTK_REAL 
PunctEvalAtArbitPositionFastSF (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi, int nvar, int n1, int n2, int n3)
{
  int i, j, k, N;
  CCTK_REAL *p, *values1, **values2, result;
...
/* --------------------------------------------------------------------------*/
CCTK_REAL
PunctIntPolAtArbitPositionFastSF (int ivar, int nvar, int n1,
			    int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
			    CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  ...
  result = PunctEvalAtArbitPositionFastSF (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
void SpecCoefSF(int n1, int n2, int n3, int ivar, CCTK_REAL *v, CCTK_REAL *cf)
{
  ...
  // VASILIS: Here v is a pointer to the values of the variable v at the collocation points and cf_v a pointer to the spectral coefficients that this routine calculates
	...
	N=maximum3SF(n1,n2,n3);
	p=dvectorSF(0,N);
	values3=d3tensorSF(0,n1,0,n2,0,n3);
	values4=d3tensorSF(0,n1,0,n2,0,n3);
		...
		for (j = 0; j < n2; j++){
	      ...
	      free_dvectorSF(p,0,N);
	      free_d3tensorSF(values3,0,n1,0,n2,0,n3);
	      free_d3tensorSF(values4,0,n1,0,n2,0,n3);
