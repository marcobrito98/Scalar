/* TwoPuncturesSF:  File  "TwoPuncturesSF.c"*/
...
/* Swap two variables */
static inline
void swapSF (CCTK_REAL * restrict const a, CCTK_REAL * restrict const b)
{
  CCTK_REAL const t = *a; *a=*b; *b=t;
}
#undef SWAP
#define SWAP(a,b) (swapSF(&(a),&(b)))



static
void set_initial_guessSF(CCTK_POINTER_TO_CONST cctkGH,
                       derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          i3D = IndexSF(ivar,i,j,k,1,n1,n2,n3);
  ...
  for (ivar = 0; ivar < nvar; ivar++)
    for (i = 0; i < n1; i++)
      for (j = 0; j < n2; j++)
        for (k = 0; k < n3; k++)
        {
          indx = IndexSF(ivar,i,j,k,1,n1,n2,n3); v.d0[indx]/=(-cos(Pih * (2 * i + 1) / n1)-1.0); }
  ...
  if (do_initial_debug_output && CCTK_MyProc(cctkGH) == 0)
  {
    ...
    for (ivar = 0; ivar < nvar; ivar++)
      for (i = 0; i < n1; i++)
        for (j = 0; j < n2; j++)
        {
          ...
          B = -cos (be);
          phi = 0.0;
          indx = IndexSF(ivar,i,j,0,1,n1,n2,n3);
    ...
    {
      CCTK_REAL d;
      indx = IndexSF(0,0,i,0,1,n1,n2,n3);
      d = PunctIntPolAtArbitPositionSF(0, nvar, n1, n2, n3, v,
              s_x[indx], 0.0, 0.0);
    ...
    {
      CCTK_REAL d;
      int ip= IndexSF(0,0,i+1,0,1,n1,n2,n3);
      indx = IndexSF(0,0,i,0,1,n1,n2,n3);
      for (j=-10; j<10; j++)
      {
        d = PunctIntPolAtArbitPositionSF(0, nvar, n1, n2, n3, v,
...
/* -------------------------------------------------------------------*/
void TwoPuncturesSF(CCTK_ARGUMENTS);
void
  ...
  * mm = par_m_minus;
  ...
  if (! F) {
    ...
    if(!(give_bare_mass)) {
      ...
      do {
        ...
        *mm = (tmp - M_p + M_m)/(2.*(1 + um));
        ...
        CCTK_ParameterSet ("par_m_plus", "TwoPuncturesSF", valbuf);
        
        sprintf (valbuf,"%.17g", (double) *mm);
        CCTK_ParameterSet ("par_m_minus", "TwoPuncturesSF", valbuf);
    ...
    *mm_adm = (1 + um) * *mm + *mp * *mm / (4. * par_b);
    ...
    admMass = (*mp + *mm
               - 4*par_b*PunctEvalAtArbitPositionSF(v.d0, 0, 1, 0, 0, nvar, n1, n2, n3));
    ...
    for (int j = imin[1]; j < imax[1]; ++j)
    {
      for (int i = imin[0]; i < imax[0]; ++i)
      {
        ...
        case GSM_Taylor_expansion:
          U = PunctTaylorExpandAtArbitPositionSF
